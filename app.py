...
# --- 0. [Master Schema] ---
# [UPDATED] 재무 관련 컬럼 추가: "예산 (KRW)", "실제 집행 비용 (KRW)"
PROJECTS_SCHEMA = [
...
def standardize_dataframe(df, schema):
    ...
    for col in schema:
        if col in df.columns:
            ...
            # [UPDATED] 스키마에 새 컬럼이 없을 경우 기본값 설정
            numeric_cols = ["용량 (MW)", "구조물 공정율", "전기 공정율", "예정 구조물", "예정 전기", 
                            "누적 구조물", "누적 전기", "총 인원", "예산 (KRW)", "실제 집행 비용 (KRW)"]
            if col in numeric_cols:
                new_df[col] = 0.0
...
if sheet:
    ...
    with tab_finance:
        ...
        if not projects_df.empty:
            ...
            st.dataframe(finance_df.style.format({
                ...
                "실제 집행 비용 (KRW)": "{:,.0f}",
                "집행률 (%)": "{:.1f}%"
            }).applymap(color_execution_rate, subset=['집행률 (%)']), use_container_width=True)
    ...
    # --- [Tab 3] 프로젝트 마스터 (기초 정보 및 재무 정보 수정) ---
    with tab3:
