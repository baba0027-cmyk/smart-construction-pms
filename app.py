...
def load_data(sheet_key):
    ...
    if not projects_df.empty:
        ...
        # [신규 기능] 공정율 컬럼이 없으면 자동으로 생성 (0으로 초기화)
...
if sheet:
    ...
    # --- 메인 탭 구성 ---
    tab1, tab2, tab3, tab4 = st.tabs(["🗺️ 지도 & 날씨", "👷 작업 현황", "📋 프로젝트 리스트 (수정 가능)", "👥 인력 관리"])
    ...
    with tab3:
        ...
        st.info("💡 표의 내용을 직접 클릭하여 수정할 수 있습니다. 수정 후 아래 버튼을 눌러 저장하세요.")
        
        # [업그레이드] st.dataframe 대신 st.data_editor를 사용하여 직접 수정 가능하게 함
        edited_projects = st.data_editor(
            projects_df, 
            use_container_width=True,
            num_rows="fixed" # 행 추가/삭제는 사이드바에서 하도록 유도
        )
